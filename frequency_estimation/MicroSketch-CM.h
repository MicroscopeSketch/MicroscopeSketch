#include "inc.h"
template<uint8_t key_len> struct MicroSketch_CM {
	...
	MicroSketch_CM(int _m, int _win, int _t, int _size_k, int _hnum, int _log_base) : m(_m), win(_win), t(_t), size_k(_size_k), hnum(_hnum), log_base(_log_base) {
		t += 2;
		n = m;
	...
	void insert(uint8_t *key, uint64_t clock) {
		...
		rep2 (k, 0, hnum) counter_add(k, hashid[k], cur);
	}
